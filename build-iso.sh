...
readonly BASE_PKGS="dialog cryptsetup lvm2 mdadm void-docs-browse xdg-user-dirs papirus-icon-theme plata-theme gsettings-desktop-schemes ntfs-3g fuse-exfat exfat-utils $GRUB"
...
readonly LXDE_PKGS="$X_PKGS lxdm  gvfs-afc gvfs-mtp gvfs-smb udisks2 network-manager-applet firefox-esr openbox pcmanfm lxde-icon-theme lxde-common lxappearance lxsession lxterminal lxlauncher lxinput lxrandr lxpanel lxtask gpickview upower"
...
if [ -z "$IMAGE" -o "$IMAGE" = fluxboxwm ]; then
	if [ ! -e $BLACKBOXWM_IMG ]; then
		./mklive.sh -a $ARCH -o $FLUXBOXWM_IMG -p "$FLUXBOXWM_PKGS" ${REPO} "$@"
	fi
fi
